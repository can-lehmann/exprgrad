...
```nim
proc identity*(a: Fun): Fun = iters it: result{it} ++= a{it}
```
...
proc `*`*(a: Fun, factor: float64): Fun =
  iters it: result{it} ++= a{it} * @factor
```
...
```nim
iters it: identity*{x} ++= inp{x} | custom_grad(
  grad(inp){x} ++= inp{x} * grad(identity){x}
)
```
